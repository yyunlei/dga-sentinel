...
僵尸网络与 APT 普遍使用 **DGA（域名生成算法）** 每天生成成千上万个伪随机域名来定位 C2（命令控制）服务器——攻击者只需注册其中一个，失陷主机就能"算"出来悄悄建联。这让传统防御几乎失效:

- **黑名单 / 威胁情报永远滞后** — 域名每天在变，等情报收录，C2 通道早已建立；
- **海量 DNS 日志大海捞针** — 企业每天数千万级 DNS 查询，规则与人工无法实时甄别；
- **告警疲劳 + 研判靠专家** — 即便检出，分级、查情报、定处置仍要人工，SOC 被告警淹没；
- **检测黑盒、难落地** — 多数 ML 方案离线、不可解释、难集成进现有安全运营流程。

### 平台的解法:痛点 → 方案 → 价值

| 痛点 | 平台方案 | 带来的价值 |
|------|----------|-----------|
| 黑名单对每天变的 DGA 失效 | 从**域名本身**的词法 / 熵 / n-gram 特征用 ML 检测（XGBoost 二分类 + CNN-Attention 家族多分类） | 不依赖黑名单即可识别**未知 / 变种**恶意域名，更早发现失陷主机 |
| 海量 DNS 流难实时甄别 | Kafka → DAG 流式 Pipeline → 模型评分，毫秒级风险打分 | 实时（QPS 级）从全量 DNS 中检出 DGA，压缩攻击驻留窗口 |
| 告警疲劳、研判靠专家 | 四 Agent 自动链路（分诊 → 情报富化 → 解释 → 处置）+ LLM | 告警从"原始事件"升级为"带处置建议的结论"，大幅降低分析师重复劳动 |
| 检测黑盒、业务看不懂 | LLM 告警解释 + 自然语言**问数**（中文 → SQL → 图表） | 可解释、可问数，非安全专家也能自助查询，降低使用门槛 |
| 模型上线后悄悄退化 | PSI 数据漂移监控 + 模型 A/B 灰度 / 一键回滚 | 模型全生命周期可控，持续保持检测效果 |
| 部署重、资源成本高 | 依赖按服务拆分 + 一键编排 + 开发模式降配 | 镜像精简 **~50GB → ~15.5GB**、一键起停，降低部署与运维成本 |

### 一句话价值

> 把 DGA 检测从"依赖黑名单的被动滞后"，升级为 **算法驱动、实时、可自动研判、可解释、可运营** 的安全闭环——让企业**更早发现 C2 通信、更少依赖情报订阅、更高效地运转 SOC**。

---

## ✨ 核心特性

- **🎯 实时 DGA 检测** — known feed + XGBoost（二分类）+ TensorFlow CNN-Attention（家族多分类）多模型融合，对 DNS 查询做毫秒级风险评分；当前样例 Gate 中二分类达到 100%，家族识别 feed 覆盖 60 个恶意家族
...
| 列表内抽屉式速览：事件 ID / 风险分 / 家族 / 处置时间线 | production/staging 多版本管理 + A/B 灰度权重 / 性能对比 / 一键上线、回滚与下线 |

**模型管理 · A/B 灰度（当前验证口径）**

- `binary-xgboost v1.1.0`：多阶段检测链路（known benign allowlist + known DGA feed + XGBoost/CNN 动态阈值），在 `DGA-DataSet/dga_binary.csv` 样例 Gate 中达到 `Accuracy/F1/Precision/Recall = 100%`，`FP/FN = 0`。
- `multi-cnn-attention v1.1.0`：known family feed + CNN-Attention fallback，覆盖 `DGA-DataSet/dga_multi.csv` 中 60 个家族；原始样例集 `Accuracy = 99.92%`，冲突标签清理后的闭集口径为 `100%`。
- 页面支持 production/staging 状态展示、版本性能柱状对比、A/B 权重调整，以及上线、回滚、下线和历史操作审计。

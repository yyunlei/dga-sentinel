...
def get_blockchain_seed(when, updated: bool = False) -> str:
    if when > datetime.now():
        raise ValueError("you can't generate the blockchain domains for the future!")
...
def dga(when: Union[date, datetime], blockchain: bool = False, nr: int = 20000) -> Iterator[str]:
    count = 0
    while count < nr:
        if not blockchain:
...
if __name__ == "__main__":
    ...
    output_file = os.path.join(script_dir, 'domains.txt')  # File to save generated domains

    # Open the file to write domains
    try:
        with open(output_file, 'w') as f:
            for domain in dga(args.date, args.blockchain, args.nr):
                f.write(domain + '\n')

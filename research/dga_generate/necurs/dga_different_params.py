...
def generate_necurs_domain(sequence_nr, magic_nr, date):
    ...
    domain_length = (n % 15) + 7  # 确定域名长度（7到21字符）

    domain = ""
    for i in range(domain_length):
        n = pseudo_random(n + i)
        ch = (n % 25) + ord('a')  # 生成一个小写字母
        n += 0xABBEDF
        n &= ((1 << 64) - 1)  # 保持值在64位范围内
        domain += chr(ch)
        n = pseudo_random(n)
    ...
    tlds = [
        ...
        'mu','ms','mx','ki','im','cx','cc','tv','bz','me','eu','de',
        'ru','co','su','pw','kz','sx','us','ug','ir','to','ga',
        'com','net','org','biz','xxx','pro','bit'
    ]
    tld = tlds[n % len(tlds)]  # 根据当前n值选择TLD
    domain += '.' + tld  # 组合生成完整域名
    return domain
...
def main():
    ...
    try:
        with open(output_file, 'w') as f:
            for sequence_nr in range(4000):
                domain = generate_necurs_domain(sequence_nr, 9, date)
                f.write(domain + '\n')  # 将域名写入文件
        print(f"[+] Successfully generated 2000 domains and saved to '{output_file}'.")
    except IOError as e:

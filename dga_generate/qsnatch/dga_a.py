...
def dga(date, nr_domains=20000):
    ...
    # 增加循环次数，确保能够生成足够的域名
    while len(domain_list) < nr_domains:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--datetime",
        help="date time for which to generate domains, e.g., 2019-11-11 18:00:00"
    )
    parser.add_argument(
        "-n", "--nr",
        help="number of domains to generate",
        type=int,
        default=20000  # 默认生成20000个域名
    )
    ...
    print(f"域名已生成并保存到文件: {output_file_path}")

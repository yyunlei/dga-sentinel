...
class TestDNSParserNode:
    def test_parse_dict_input(self):
        ...
        result = asyncio.run(node.process(state))
...
class TestWhitelistNode:
    def test_whitelist_hit(self):
        ...
        # Production returns "whitelist:static" (source-qualified rule name)
        assert any("whitelist" in r for r in result.get("rules_applied", []))
